...
# -*- coding: utf-8 -*-
"""
Sample and convert the Jigsaw Unintended Bias dataset 
...
def main():
    ...
    ap.add_argument("--data_dir", required=True, help="Folder with Kaggle Jigsaw CSV (train.csv)")
    ap.add_argument("--out", required=True, help="Output Parquet/CSV file path")
    ap.add_argument("--sample", type=int, default=50000, help="Number of samples to keep")
    args = ap.parse_args()

    train_path = Path(args.data_dir) / "train.csv"
    if not train_path.exists():
        raise FileNotFoundError(f"{train_path} not found. Make sure you downloaded it from Kaggle.")
